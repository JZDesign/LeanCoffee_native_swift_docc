...
struct LeanCoffeeView: View {
    ...
    var addTopicButton: some View {
        NavigationLink(destination: {
            CreateTopicView(presenter.leanCoffee)
                .onDisappear {
                    Task {
    ...
    func errorMessageProvider(_ error: Error?) -> String? {
        switch error as? HTTPClientError {
        case .requestError(let code, _, _):
            if code == 409 {
                return "You've already voted on this topic!"
            }
        default: return nil
        }
    ...
    var body: some View {
        ...
        Text(presenter.leanCoffee.title)
            ...
            .multilineTextAlignment(.center)
            .lineLimit(2)
            .navigationBarItems(trailing: addTopicButton)
        ...
        .toAsyncView(presenter, errorMessageProvider: errorMessageProvider)
    }
}
